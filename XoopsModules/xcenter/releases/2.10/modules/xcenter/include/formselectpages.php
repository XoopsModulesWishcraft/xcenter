...
class XoopsFormSelectPages extends XoopsFormElement
{
	...
	function GetPages($ownid){
		...
		$xcenters = $xcenter_handler->getObjects(new Criteria('parent_id', 0), true);
		$langs_array = $this->TreeMenu(array(), $xcenters, -1, $ownid);
		return $langs_array;
	}
	
	function TreeMenu($langs_array, $xcenters, $level, $ownid) {
		$level++;
		$xcenter_handler =& xoops_getmodulehandler(_XTR_CLASS_XCENTER, _XTR_DIRNAME);
		foreach($xcenters as $storyid => $xcenter) {
			if ($storyid!=$ownid) {
				$langs_array[$storyid] = str_repeat('--', $level).xcenter_getTitle($storyid);
				if ($xcentersb = $xcenter_handler->getObjects(new Criteria('parent_id', $storyid), true)){
					$langs_array = $this->TreeMenu($langs_array, $xcentersb, $level);
				}
			}
		}
		$level--;
