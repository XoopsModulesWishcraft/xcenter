...
function xoops_module_update_xcenter(&$module) {
	
	if ($module->getVar('version')<=210) {
				
		$text_handler =& xoops_getmodulehandler('text', 'xcenter');
		
		$texts = $text_handler->getObjects(NULL, false);
		
		foreach($texts as $xcenterid => $text) {
